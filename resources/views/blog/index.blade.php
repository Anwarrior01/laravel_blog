...
@section('content')
    ...
    @if (Auth::check())
    <div class="sm:grid  mx-auto py-16 px-5 border-b border-gray-300 ">
        <a href="/blog/create"
            class="bg-gray-600 py-3 px-5 text-gray-100 rounded-lg uppercase text-lg place-self-start mt-4">Create Post</a>
    </div>
     @endif
    @foreach ($posts as $post)
        <div class="sm:grid grid-cols-2 gap-16 mx-auto py-16 px-5 border-b border-gray-300 ">
            ...
            <div>
                ...
                <div>on: <span class="italic text-gray-500">{{ date('d-m-Y', strtotime($post->updated_at)) }}</span></div>
                <p class="text-l text-gray-700 py-8 leading-6">{{ $post->description }}</p>
                <a href="/blog/{{ $post->slug }}"
                    class="bg-gray-700 py-3 px-5 text-gray-100 rounded-lg uppercase text-lg place-self-start mt-4">read
                    more</a>
            </div>
        </div>
    @endforeach
